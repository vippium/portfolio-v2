...
export const ExpandableDescription: React.FC<Props> = ({ description }) => {
  ...
  useEffect(() => {
    if (pRef.current) {
      setIsOverflowing(pRef.current.scrollHeight > 85);
    }
  }, [description]);
  ...
  return (
    <div className="relative mt-4">
      ...
      {!expanded && isOverflowing && (
        <div className="absolute bottom-0 left-0 right-0 h-8 bg-gradient-to-t from-zinc-900 via-zinc-900/80 to-transparent pointer-events-none" />
      )}
    </div>
  );
};
